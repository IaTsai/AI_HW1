...
\author{Ian Tsai \\ Student ID: 313553058}
\date{}
...
\textbf{GitHub Repository:} \url{https://github.com/IaTsai/AI_HW1}\\
{\small Dataset: \texttt{data/raw/reviews.csv} \quad Documentation: \texttt{DATASET.md}}

% ══════════════════════════════════════════════════════════
\section{Research Question and Motivation}

Can we automatically classify the sentiment of Taiwanese restaurant reviews from Google Maps using both traditional ML and deep learning approaches?
...
This project aims to (1)~construct a novel dataset of Taiwanese food reviews via web crawling, (2)~compare multiple supervised learning methods, and (3)~investigate how the ambiguous 3-star class affects classification performance.

\textbf{Model architecture overview.}
I experiment with three categories of supervised methods:
\begin{enumerate}[nosep]
  \item \textbf{Non-deep-learning:} TF-IDF features + Logistic Regression (LR) and SVM classifiers.
  \item \textbf{Transfer learning (non-DL classifier):} I use a frozen pretrained XLM-RoBERTa-large~\citep{conneau2020xlmr} purely as a \textit{feature extractor}---producing 1{,}024-dimensional embeddings via mean pooling---then feed these into my trained LR/SVM classifiers. The BERT model is \textbf{not fine-tuned}; the trainable component is a classical ML classifier.
  \item \textbf{Deep learning:} (a)~BERT zero-shot classification (no training on my data), and (b)~a PyTorch MLP (multi-layer perceptron) trained on the BERT embeddings, where the neural network itself is the learned classifier.
\end{enumerate}
I also demonstrate a practical application: a tool that analyzes any restaurant's Google Maps reviews to produce sentiment trend reports for restaurant owners.
...
The dataset exhibits severe class imbalance: positive reviews dominate at 79.3\%, while neutral and negative each constitute $\sim$10\%.
...
I constructed 45 search queries spanning 10 cities and 8 night markets across Taiwan.
The Playwright crawler searches Google Maps, iterates through restaurants, scrolls to load reviews, expands truncated text, and extracts structured data.
Anti-blocking measures include random delays (2--5s), restaurant rotation, and randomized query order.
...
My approach evolves through three stages, from a traditional NLP baseline to BERT-based transfer learning.
I use two non-deep-learning classifiers (LR, SVM) and two deep-learning approaches (BERT zero-shot, MLP), satisfying the requirement of at least two methods with at most one deep-learning based.

\subsection{Stage 1: TF-IDF + Classical ML}

\textbf{Tokenization.}
Chinese text requires word segmentation.
I use jieba~\citep{jieba} in precise mode with a custom dictionary containing domain-specific terms (e.g., ``CP值'' [value for money], ``踩雷'' [bad experience], ``排隊'' [queuing]).
Stopwords are filtered using a curated Chinese stopword list, and tokens with length $\leq 1$ are removed.

\textbf{Feature extraction.}
I apply TF-IDF vectorization (\texttt{max\_features}=5{,}000) followed by TruncatedSVD~\citep{halko2011svd} for dimensionality reduction to 150 dimensions, which retains 25.2\% of the total variance.

\textbf{Classifiers.}
I train two models: (1)~Logistic Regression (LR) with L2 regularization (\texttt{max\_iter}=1000), and (2)~SVM with RBF kernel (\texttt{C}=1.0, $\gamma$=\texttt{scale}).
...
To capture deeper semantic features, I use XLM-RoBERTa-large~\citep{conneau2020xlmr} fine-tuned on XNLI~\citep{conneau2018xnli} (\texttt{joeddav/xlm-roberta-large-xnli}) as a \textit{feature extractor}.
I perform mean pooling over the last hidden states to obtain 1{,}024-dimensional embeddings for each review.
These embeddings replace TF-IDF+SVD features while keeping my trained LR/SVM classifiers---this constitutes \textbf{transfer learning}, where BERT provides the representation and my classifiers are the learned components.

I also run BERT in a \textbf{zero-shot} configuration as a baseline, classifying reviews into ``positive,'' ``neutral,'' and ``negative'' without any training on my data.

\subsection{Stage 2b: MLP Classifier (Deep Learning)}

To provide a true deep-learning classifier, I train a PyTorch MLP on the same BERT embeddings.
The architecture is: \texttt{Linear(1024,256)} $\to$ \texttt{BatchNorm} $\to$ \texttt{ReLU} $\to$ \texttt{Dropout(0.3)} $\to$ \texttt{Linear(256,64)} $\to$ \texttt{BatchNorm} $\to$ \texttt{ReLU} $\to$ \texttt{Dropout(0.2)} $\to$ \texttt{Linear(64,$n$)}.
I use Adam optimizer ($lr$=1e-3, weight decay=1e-4) with cosine annealing, class-weighted cross-entropy loss, and train for 50 epochs (80 for the final model).
Unlike BERT+LR where only a linear classifier is learned, the MLP \textbf{learns non-linear decision boundaries} in the embedding space, making it a genuine deep-learning method.

\subsection{Stage 3: Binary Classification (3-Star as Negative)}

Based on my experiment (Section~\ref{sec:neutral}), I reclassify 3-star reviews as negative, reducing the task to binary (satisfied vs.\ dissatisfied).
...
I use Macro F1 as the primary metric since it accounts for class imbalance.
...
BERT zero-shot\textsuperscript{\dag}   & 0.815 & 0.559 & ---   & ---   \\
BERT + SVM       & 0.798 & 0.328 & 0.599 & 0.350 \\
BERT + MLP\textsuperscript{\dag}  & 0.779 & 0.619 & 0.598 & 0.664 \\
\textbf{BERT + LR} & \textbf{0.842} & \textbf{0.636} & \textbf{0.652} & \textbf{0.625} \\
\bottomrule
\end{tabular}
\end{table}

{\footnotesize \textsuperscript{\dag}Deep-learning methods.}

All models achieve $\sim$82\% accuracy, largely because predicting ``positive'' for everything already yields 79.3\%.
The key differentiator is Macro F1: \textbf{BERT+LR outperforms all others at 0.636}, surpassing the zero-shot baseline (0.559) and the deep-learning MLP (0.619).
The MLP's non-linear layers do not outperform LR---the BERT embedding space is already well-structured for linear separation.
BERT+SVM performs worst (F1=0.328)---the RBF kernel struggles in the 1{,}024-dimensional BERT space.
...
I train LR with 10\%, 25\%, 50\%, 75\%, and 100\% of the data to study how performance scales.
...
I compare three strategies to address the 79:10:10 class imbalance.
...
SMOTE produces similar results. The accuracy drop reflects improved minority-class recall at the cost of some majority-class errors.
...
I test four scenarios to understand its impact.
...
I apply noise injection (random character deletion/substitution) to augment minority classes.
...
Combining the best strategies (BERT embeddings + binary classification), I achieve the overall best model.
...
BERT + MLP (binary)\textsuperscript{\dag} & 0.849 & 0.787 & 0.771 & 0.812 \\
\textbf{BERT + LR (binary)} & \textbf{0.892} & \textbf{0.826} & \textbf{0.846} & \textbf{0.811} \\
BERT + LR (binary, balanced) & 0.849 & 0.792 & 0.771 & 0.826 \\
\bottomrule
\end{tabular}

{\footnotesize \textsuperscript{\dag}Deep-learning method.}
\end{table}

The final \textbf{BERT+LR binary model achieves F1=0.826}, an 85\% improvement over the TF-IDF baseline.
The MLP (F1=0.787) outperforms TF-IDF methods but falls short of LR, again confirming that linear classifiers are optimal for BERT embeddings.
...
To demonstrate practical value, I built a tool (\texttt{analyze\_restaurant.py}) that, given a Google Maps URL, automatically: (1)~scrapes all reviews via multi-pass crawling with deduplication, (2)~predicts sentiment using the best model (BERT+LR binary), and (3)~generates monthly trend reports.
...
The tool identified a satisfaction dip to 35.3\% in August 2025 (recovered after two months) and recurring complaints about hygiene and specific employees---actionable insights for the owner.

% ══════════════════════════════════════════════════════════
\section{Discussion}

\textbf{Are the results what I expected?}
Yes and no.
I expected BERT embeddings to outperform TF-IDF (confirmed: +42\% F1), but the magnitude of BERT+SVM's failure (worst overall) was surprising.
The RBF kernel's reliance on Euclidean distance does not suit the high-dimensional, semantically structured BERT space.
Interestingly, the deep-learning MLP (F1=0.619) also falls short of the simpler LR (F1=0.636), suggesting that BERT embeddings are already linearly separable and additional non-linear layers add noise rather than signal.

I also expected 3-class classification to be harder than binary, but the specific finding that ``3-star as negative'' outperforms ``remove neutral'' was a genuinely useful cultural insight.
...
\textbf{What would I do with more time?}
\begin{enumerate}[nosep]
  \item Fine-tune BERT on my dataset (currently only used as a frozen feature extractor) to potentially push F1 beyond 0.9.
...
Why does BERT+SVM perform worst despite using the same rich embeddings as BERT+LR? I hypothesize the RBF kernel's Euclidean distance metric is ill-suited for the high-dimensional, semantically clustered BERT space, but a rigorous theoretical analysis is needed.
Additionally, does the ``3-star = negative'' cultural pattern generalize to other East Asian review platforms (e.g., Tabelog in Japan, Dianping in China), or is it specific to Taiwanese Google Maps users?
Finally, would fine-tuning BERT on this domain-specific corpus close the gap between MLP and LR, or would the linear separability of the embedding space persist?
...
\subsection*{A.7 MLP Classifier (\texttt{train\_mlp.py}) --- excerpt}
\inputminted[firstline=1,lastline=100]{python}{code/train_mlp.py}

\subsection*{A.8 Restaurant Analyzer (\texttt{analyze\_restaurant.py}) --- excerpt}
